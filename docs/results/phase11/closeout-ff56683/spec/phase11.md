...
- [x] Applicable Phase 1–10 regressions and `make check` remain green (full clean `make check`, 238 host tests).
- [x] 31.25 MHz routed overlay and physical Pynq-Z1 acceptance at the Phase 9/10 rigor.
- [x] Self-contained closeout bundle and read-only audit.

The README Phase 11 exit is satisfied at commit `ff56683` plus the retained
closeout documentation: the frozen INT8 MLP runs bit-exactly through the
scalar, multicore, DOT8 and NPU paths, the four-path study and physical
captures agree, and the [self-contained closeout
bundle](results/phase11/closeout-ff56683/README.md) passes
