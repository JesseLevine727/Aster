...
- [x] Actual CPU-controlled RAM-backed C GEMM on one/two harts, cache off/on and supported waits.
- [x] DMA publication/coherence, safe global STOP/ABORT, reset/error/timeout and disabled/legacy tests.
- [x] AsterBench v7 records, independent output/RAM/event/provenance audits, mutation tests and fresh repeats.
- [x] 31.25 MHz routed/reset/HWH/resource/bitstream gates and guarded PYNQ Linux execution where applicable.
- [x] Applicable Phase 1–8 regressions, immutable requirement mapping, fresh checkout and pushed closeout.

The README Phase 9 exit is satisfied at commit `2493435` plus the retained
closeout documentation: randomized INT8 GEMM is bit-correct against the
independent scalar model, all applicable gates above pass, AsterBench v7 is
audited, and both cache configurations execute through PYNQ Linux/PCAP on the
Pynq-Z1. Area, frequency, utilization and speedup claims are retained as
simulation/FPGA measurements; they are not inferred from a unit test.
