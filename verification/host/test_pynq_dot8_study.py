...
import pynq_dot8_study as batch
...
def report(entry,prior,prior_sha,loaded,sources):
    c = entry["configuration"]; records = rows(name=c["name"],k=c["k"],alignment=c["alignment"])
    for r in records:
        r["l1"] = c["l1"]
        if not c["l1"]:
            for h in range(2):
    ...
    return dict(previous_bitstream=prior,previous_bitstream_sha256=prior_sha,loaded_bitstream=loaded,downloaded=prior != loaded,
        ...
        boots=[dict(boot=i,records=copy.deepcopy(records),reference_comparison=dict(exact_counter_match=True)) for i in (1,2)])


class PhysicalDot8Study(unittest.TestCase):
    def test_full_plan_and_isolated_imports(self):
        entries = batch.schedule()
        self.assertEqual(len(entries),174); self.assertEqual(len({e["id"] for e in entries}),174)
        self.assertEqual([e["configuration"]["l1"] for e in entries],[0]*87+[1]*87)
        self.assertEqual([e["configuration"]["k"] for e in entries if e["fresh_repeat_of"]],[128,32,32]*2)
        with tempfile.TemporaryDirectory(prefix="aster-pynq8-study-import-") as directory:
            root = Path(directory)
            for name in batch.DRIVER_FILES: shutil.copy2(ROOT/"scripts"/name,root/name)
            subprocess.run([sys.executable,"-I","-c",
                "import sys; sys.path.insert(0,sys.argv[1]); import pynq_dot8_study; assert 'pynq' not in sys.modules",str(root)],
                cwd=root,check=True,capture_output=True)

    def test_capture_all_cases_hash_chain_and_partial_failure_retention(self):
        for failure in (False,True):
            with self.subTest(failure=failure), tempfile.TemporaryDirectory(prefix="aster-pynq8-study-flow-") as directory, ExitStack() as stack:
                ...
                if failure:
                    ...
                    self.assertEqual(len(calls),174); audited.assert_called_once()
                    self.assertEqual([c.download for c in calls],[False]*87+[True]+[False]*86)
                    self.assertEqual([c.expected_loaded_sha256 for c in calls],["1"*64]*88+["2"*64]*86)
                    self.assertEqual((m["summary"]["boots"],m["summary"]["paired_jobs"],m["summary"]["method_records"]),(348,1392,2784))
                    self.assertTrue(all(r["identical_records"] for r in m["summary"]["repeats"]))
                    # All fixture custom methods slow down; no fabricated crossover.
                    self.assertTrue(all(s["first_any_pair_custom_win_k"] is None for s in m["summary"]["series"].values()))
    ...
    def test_audit_coverage_summary_hashes_and_rehashed_sequence_mutations(self):
        with tempfile.TemporaryDirectory(prefix="aster-pynq8-study-audit-") as directory, ExitStack() as stack:
            ...
            bad = copy.deepcopy(m); bad["summary"]["method_records"] = 2784.0; mutations.append(bad)
            bad = copy.deepcopy(m); bad["summary"]["series"]["dot-aaligned-c0"]["first_any_pair_custom_win_k"] = 1; mutations.append(bad)
