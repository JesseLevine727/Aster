...
import asterbench_dot8 as b
import dot8_study as study
from test_asterbench_dot8 import rows


class Dot8StudyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.captures = {}
        for entry in study.plan():
            c = entry["configuration"]
            records = rows(name=c["name"], k=c["k"], alignment=c["alignment"])
            for r in records:
                r["l1"] = c["l1"]
                if not c["l1"]:
                    for h in range(2):
                        for event in ("i_access","i_miss","d_access","d_miss","writeback"): r[f"h{h}_{event}"] = 0
            capture = dict(configuration=dict(c), records=[copy.deepcopy(records),copy.deepcopy(records)],
                metadata=dict(dirty=False,revision="0"*40,source_sha256="0"*64,build_command=["make","--no-print-directory","-j2","BUILD_DIR=/test/build"]),
                toolchain=dict(tools=dict(gcc=dict(sha256="0"*64))),observations=[],stops=[],symbols={},
                artifacts={key:dict(file=key+".bin",sha256="0"*64) for key in ("firmware","ram1","ram2")})
    ...
    def test_frozen_full_coverage_and_independent_repeat_selection(self):
        entries = study.plan(); self.assertEqual(len(entries), 174)
        main = [e for e in entries if e["fresh_repeat_of"] is None]
        repeats = [e for e in entries if e["fresh_repeat_of"]]
        self.assertEqual(len(main), 168); self.assertEqual(len(repeats), 6)
        self.assertEqual({(e["configuration"]["name"],e["configuration"]["k"], e["configuration"]["alignment"], e["configuration"]["l1"]) for e in main},
                         {(name,k,alignment,cache) for name,sizes in b.SIZES.items() for k in sizes for alignment in b.ALIGNMENTS for cache in (0,1)})
        self.assertTrue(all(e["configuration"]["alignment"] == "aligned" and e["configuration"]["k"] == (128 if e["configuration"]["name"] == "dot" else 32) for e in repeats))
        summary = study.summarize(self.captures)
        self.assertEqual((summary["boots"], summary["paired_jobs"], summary["method_records"]), (348,1392,2784))
        with self.assertRaises(ValueError): study.summarize({})

    def test_crossover_keeps_mixed_pairs_slowdowns_ties_reversals(self):
        def point(size, low, high): return dict(k=size, min_scalar_over_custom=low, max_scalar_over_custom=high)
        ...
        self.assertEqual(s["first_any_pair_custom_win_k"], 2)
        self.assertEqual(s["first_all_pairs_custom_win_k"], 4)
        self.assertEqual(s["later_not_all_pairs_win_k"], [8,16])
        self.assertEqual(s["all_pairs_win_through_largest_tested_from_k"], 32)
        for points in ([point(0,2,2),point(1,.5,.8)], [point(0,2,2),point(1,1,1)]):
            s = study.series_summary(points)
            self.assertIsNone(s["first_any_pair_custom_win_k"])
            self.assertIsNone(s["first_all_pairs_custom_win_k"])
            self.assertIsNone(s["all_pairs_win_through_largest_tested_from_k"])
    ...
    def test_full_read_only_audit_and_manifest_mutations(self):
        with tempfile.TemporaryDirectory(prefix="aster-dot8-study-fixture-") as directory:
            ...
            with mock.patch.object(study.results, "load", side_effect=lambda p,**kw:self.captures[p.stem]) as loader:
                study.audit(path); self.assertEqual(loader.call_count, 174)
                mutants = []
                for key in original:
                ...
                bad = copy.deepcopy(original); bad["summary"]["series"]["dot-aaligned-c0"]["first_any_pair_custom_win_k"] = 1; mutants.append(bad)
    ...
    def test_mixed_capture_source_tools_geometry_and_builds(self):
        with tempfile.TemporaryDirectory(prefix="aster-dot8-study-mixed-") as directory:
