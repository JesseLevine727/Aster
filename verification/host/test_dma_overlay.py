...
import coherent_overlay as legacy
from test_coherent_overlay import evidence as legacy_evidence


def evidence(cache):
    data = legacy_evidence(cache)
    hwh = ET.fromstring(data["aster_linux.hwh"])
    ET.SubElement(hwh.find(".//MODULE[@INSTANCE='aster']/PARAMETERS"), "PARAMETER", NAME="ENABLE_DMA", VALUE="1")
    data["aster_linux.hwh"] = ET.tostring(hwh)
    data["build.log"] = data["build.log"].replace(f"/fixture/build 2 1 {int(cache)}\n".encode(),
                                                f"/fixture/build 2 1 {int(cache)} 1\n".encode())
    return data
...
def manifest(root, cache, data):
    ...
    return dict(schema=overlay.SCHEMA, revision="b"*40, dirty=False, source_files=sources,
                ...
                source_worktree="/fixture/source", build_directory="/fixture/build", harts=2, caches=cache, dma=True,
                files={name: dict(sha256=hashlib.sha256(raw).hexdigest(), bytes=len(raw)) for name, raw in data.items()},
                handoff=overlay.validate_handoff(root/"aster_linux.hwh", 2, expected_coherent=True, expected_cache=cache, expected_dma=True),
                signoff=overlay.signoff(data))


class DmaOverlay(unittest.TestCase):
    def test_explicit_dma_package_and_unchanged_legacy_gate(self):
        self.assertIs(overlay.signoff, legacy.signoff)
        self.assertEqual(overlay.FILES, legacy.FILES)
        for cache in (False, True):
            with self.subTest(cache=cache), tempfile.TemporaryDirectory(prefix="aster-dma-overlay-") as directory:
                ...
                self.assertTrue(overlay.audit(path, clean=False)["handoff"]["dma"])
                ...
                for key, value in (("dirty", True), ("harts", True), ("harts", 1), ("caches", int(cache)), ("dma", 1),
    ...
    def test_rehashed_dma_identity_and_signoff_mutations(self):
        with tempfile.TemporaryDirectory(prefix="aster-dma-overlay-mutate-") as directory:
            root = Path(directory); data = evidence(True)
            for name, raw in data.items(): (root/name).write_bytes(raw)
            original = manifest(root, True, data); path = root/"overlay.json"
            changes = [("build.log", b"2 1 1 1\n", b"2 1 1\n"), ("build.log", b"2 1 1 1\n", b"2 1 1 0\n"),
                       ("build.log", b"2 1 1 1\n", b"1 1 1 1\n"), ("build.log", b"2 1 1 1\n", b"2 1 1 1 1\n"),
                       ("aster_linux.hwh", b'NAME="ENABLE_DMA" VALUE="1"', b'NAME="ENABLE_DMA" VALUE="0"'),
                       ("aster_linux.hwh", b'NAME="ENABLE_DMA"', b'NAME="MISSING_DMA"'),
    ...
    def test_symlinks_and_missing_artifacts_rejected(self):
        with tempfile.TemporaryDirectory(prefix="aster-dma-overlay-links-") as directory:
