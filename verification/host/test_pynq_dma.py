...
from test_dma_results import fixture, ram_fixture
from test_asterbench_dma import stream


class PhysicalMMIO(MMIO):
    def __init__(self,rows,ram,scenario="complete"):
        super().__init__(); self.rows = rows; self.payload = stream(rows); self.cursor = 0; self.scenario = scenario
        self.ram = list(struct.unpack("<16384I",ram))
        self.registers.update({0x1c:0x70001,0x44:5+2*rows[0]["l1"],0x80:1,0x9c:5})
    ...
    def read(self,address):
        if self.registers[0]:
            ...
            final = self.rows[-1]
            values = {0x84:final["raw_dma_status"],0x88:final["raw_dma_bytes_done"],0x8c:0,
                      0x90:final["raw_dma_job_cycles"] & bench.U32,0x94:final["raw_dma_job_cycles"] >> 32,0x98:0}
            for i,key in enumerate(bench.DMA_EVENTS):
                value = final["dma_"+key]
                values.update({0xa0+i*8:value & bench.U32,0xa4+i*8:value >> 32})
            if address in values:
                return values[address]+(self.scenario == "bad-dma" and address == 0x88)
...
def inputs(root,cache=True,jobs=4):
    reference,_,data = fixture(caches=int(cache),jobs=jobs)
    reference["metadata"]["dirty"] = False
    for key,value in data.items(): (root/reference["artifacts"][key]["file"]).write_bytes(value)
    ...
    (root/"aster_linux.hwh").write_bytes(dma_overlay(cache)["aster_linux.hwh"])
    ...
    hardware = dict(caches=cache,dma=True,dirty=False,source_files=dict(reference["metadata"]["source_files"]),
...
class PynqDma(unittest.TestCase):
    def test_no_download_reuses_only_exact_idle_dma_image(self):
        with tempfile.TemporaryDirectory(prefix="aster-pynq7-reuse-") as directory, ExitStack() as stack:
    ...
    def test_changed_preflight_inputs_and_second_guard_are_nonmutating(self):
        for scenario in ("firmware","missing-download","second-loaded","second-busy","second-manifest"):
            with self.subTest(scenario=scenario), tempfile.TemporaryDirectory(prefix="aster-pynq7-race-") as directory, ExitStack() as stack:
                root = Path(directory); args,reference,hardware,data = inputs(root)
    ...
    def test_isolated_shipping_closure_never_imports_pynq(self):
        with tempfile.TemporaryDirectory(prefix="aster-pynq7-import-") as directory:
            root = Path(directory)
            for name in physical.COLLECTOR_FILES: shutil.copy2(ROOT/"scripts"/name,root/name)
            subprocess.run([sys.executable,"-I","-c",
                "import sys; sys.path.insert(0,sys.argv[1]); import run_pynq_dma; assert 'pynq' not in sys.modules",str(root)],
                cwd=root,check=True,capture_output=True)

    def test_real_boot_flow_frozen_dma_and_safe_failure_cleanup(self):
        scenarios = ("complete","bad-pop","timeout","trailing","counter-mismatch","bad-ram","bad-dma","download-failed","wrong-identity")
        for cache in (False,True):
            for scenario in scenarios:
                with self.subTest(cache=cache,scenario=scenario), tempfile.TemporaryDirectory(prefix="aster-pynq7-flow-") as directory, ExitStack() as stack:
                    ...
                    if scenario == "bad-ram": ram = ram[:0x8000]+bytes(432)+ram[0x8000+432:]
                    ...
                    self.assertEqual(report["boots"][0]["before_stop"]["dma"]["status"],2)
                    ...
                    for ext in ("uart","ram"):
                        ...
                        else: changed = bytearray(saved.replace(b"h0_cycles=1000000",b"h0_cycles=1000001",1))
    ...
    def test_previous_guard_rejects_changes_without_writes_or_download(self):
        for scenario in ("loaded","hash","hwh","map","clock","busy","uart","identity","board"):
            with self.subTest(scenario=scenario), tempfile.TemporaryDirectory(prefix="aster-pynq7-guard-") as directory:
    ...
    def test_invalid_inputs_do_not_import_board_api(self):
        for key,value in (("host_pause",float("nan")),("timeout",0),("collector_revision","dirty"),
            ...
            with tempfile.TemporaryDirectory(prefix="aster-pynq7-preflight-") as directory, mock.patch.dict(sys.modules,{"pynq":None}):
    ...
    def test_physical_configuration_and_hardware_source_contract(self):
        with tempfile.TemporaryDirectory(prefix="aster-pynq7-compatible-") as directory:
