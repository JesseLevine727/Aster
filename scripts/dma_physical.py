"""Read-only contracts for physical AsterBench v5, including raw DMA diagnostics."""
import argparse
import hashlib
import math
...
COLLECTOR_FILES = {"asterbench.py", "asterbench_coherent.py", "asterbench_dma.py", "bench_results.py",
                   ...
                   "dma_results.py", "dma_physical.py", "run_pynq_dma.py"}
...
def compatible(reference, hardware):
    ...
    bench.require(hardware["dma"] is True and c["l1"] == int(hardware["caches"]), "DMA/cache hardware mismatch")
...
def stopped_state():
    return dict(control=0, status=0, hart_status=0, stop_status=1, dma=zero_dma())


def expected_dma(row):
    return dict(status=row["raw_dma_status"], bytes_done=row["raw_dma_bytes_done"], error_code=0, counting=0,
                job_cycles=row["raw_dma_job_cycles"], counters=[row["dma_"+key] for key in bench.DMA_EVENTS])
...
def validate_boot(boot, reference, directory):
    ...
    bench.require(type(boot) is dict and set(boot) == fields and boot["status"] == "PASS", "incomplete physical DMA boot")
    ...
    bench.require(type(before) is dict and set(before) == {"control","status","hart_status","stop_status","dma",
    ...
    for key,value in dict(control=1,status=1,hart_status=1,stop_status=0,tx_bytes=len(data["uart"]),
                          rx_bytes=len(data["uart"]),fifo_count=0,dma=expected_dma(records[-1])).items():
    ...
    bench.require(bench.typed_equal(boot["after_stop"],stopped_state()), "CPU/DMA did not safely reset after drain/flush")
...
def audit(path, reference_path, overlay_path, *, clean=True):
    ...
    bench.require(bench.typed_equal(report["final_state"],stopped_state()), "physical CPU/DMA not safely STOPPED")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"PASS: physical DMA raw UART/RAM/counters/provenance, {len(report['boots'])} warm boots")
