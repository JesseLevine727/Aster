#!/usr/bin/env python3
"""Run/audit all 174 predeclared scalar/custom captures physically, cache-off then on."""
import argparse
...
import run_pynq_dot8 as collector

SCHEMA = "aster.dot8.physical-study.v1"
DRIVER_FILES = physical.COLLECTOR_FILES | {"dot8_study.py", "pynq_dot8_study.py"}
...
def preflight(reference, overlays, *, clean):
    bench.require(set(overlays) == {0,1}, "both DOT8 cache overlays required")
    source = study.audit(reference,clean=clean)
    hardware = {c:dot8_overlay.audit(path,clean=clean) for c,path in overlays.items()}
...
def summary(reports):
    ...
    for entry in specs:
        ...
        point = dict(name=entry["configuration"]["name"], k=entry["configuration"]["k"],id=identifier,min_scalar_over_custom=min(ratios),max_scalar_over_custom=max(ratios),
        ...
        if entry["fresh_repeat_of"]:
            ...
            c = entry["configuration"]; series.setdefault(f"{c['name']}-a{c['alignment']}-c{c['l1']}",[]).append(point)
    return dict(captures=len(specs),boots=sum(len(r["boots"]) for r in reports.values()),
        ...
        interpretation="Physical PYNQ UART/RAM/scalar/custom-counter measurements; ratio below one is a DOT8 slowdown. Crossover is sampled, not universal or interpolated.",
        window="dispatch_load_pack_compute_store includes dispatch, loads, packing/gather, loops/tails, multiply, accumulation and stores; excludes preparation/check/UART/stop",
        cache_policy="prepared_reinitialize at fixed A/B/Y bases; not cold-cache; balanced method order retained",
        scope="Four packed lanes do not imply fourfold end-to-end speedup. No instruction-only, NPU or Fmax claim.")
...
def audit(path,reference,overlays,*,clean=True):
    ...
    bench.require(type(m) is dict and set(m) == fields and m["schema"] == SCHEMA and m["plan"] == study.PLAN and m["status"] == "complete",
                  "incomplete/unknown physical DOT8 study")
    ...
    dot8_results.digest(m["initial_bitstream_sha256"])
    ...
    for actual,spec in zip(entries,expected):
        ...
        dot8_results.digest(actual["sha256"]); bench.require(sha(target) == actual["sha256"], "physical report changed")
...
def run(args):
    ...
    dot8_results.digest(args.expected_loaded_sha256)
    _source,hardware = preflight(reference,overlays,clean=False)  # All 174 cases before any PYNQ import/write.
    ...
    try:
        for index,entry in enumerate(specs,1):
            ...
            print(f"DOT8_PHYSICAL_STUDY {index}/{len(specs)} {identifier}",flush=True)
    ...
    print(f"PASS: complete physical DOT8 study, 174 captures / 348 warm boots / 1392 paired jobs, {path}",flush=True)
    return m
...
if __name__ == "__main__":
    ...
    try:
        if args.action == "capture": run(args)
            ...
            print("PASS: complete physical DOT8 study and Git audit")
