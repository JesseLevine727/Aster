#!/usr/bin/env python3
"""Archive/audit Phase 8 DOT8 FPGA evidence without changing Phase 6's ABI.

Routed/reset signoff is shared with the proven Phase 6 gate. The envelope,
seven-argument build invocation and DOT8 HWH identity are explicitly distinct.
...
SCHEMA = "aster.dot8.overlay.v1"
FIELDS = {"schema", "revision", "dirty", "source_files", "source_sha256", "source_worktree",
          "build_directory", "harts", "caches", "dma", "dot8", "files", "handoff", "signoff"}
REQUIRED_SOURCES = {"rtl/core/aster_pcpi_dot8.sv", "rtl/peripherals/aster_dot8_perf.sv", "rtl/core/aster_atomic_hart.sv", "Makefile", "rtl/dma/aster_dma_engine.sv", "rtl/interconnect/aster_dma_arbiter.sv",
...
def audit(path, *, clean=True):
    ...
    bench.require(type(m) is dict and set(m) == FIELDS and m["schema"] == SCHEMA, "invalid DOT8 overlay envelope")
    ...
    bench.require(type(sources) is dict and REQUIRED_SOURCES <= set(sources) and
                  ...
                  "invalid/incomplete DOT8 source manifest")
    ...
    bench.require(type(m["caches"]) is bool and m["dma"] is True and m["dot8"] is True, "invalid DOT8 hardware topology")
    ...
    bench.require(len(commands) == 1 and shlex.split(commands[0]) ==
                  [m["source_worktree"], m["build_directory"], "2", "1", str(int(m["caches"])), "1", "1"],
                  "wrong logged DOT8 build inputs")
    ...
    handoff = validate_handoff(path.parent/"aster_linux.hwh", 2, expected_coherent=True,
                               expected_cache=m["caches"], expected_dma=True, expected_dot8=True)
    bench.require(bench.typed_equal(handoff, m["handoff"]), "claimed DOT8 handoff differs from HWH")
...
def capture(worktree, build, log, output, caches):
    ...
    handoff = validate_handoff(paths["aster_linux.hwh"], 2, expected_coherent=True, expected_cache=caches, expected_dma=True, expected_dot8=True)
    ...
    manifest = dict(schema=SCHEMA, revision=revision, dirty=False, source_files=sources,
        ...
        build_directory=str(build), harts=2, caches=caches, dma=True, dot8=True, handoff=handoff, signoff=validated,
    ...
    audit(path)
    print(f"PASS: archived/audited DOT8 FPGA overlay, {path}")
    return manifest
...
def main():
    ...
    try:
        ...
        else: audit(args.manifest); print("PASS: DOT8 overlay source/artifact/signoff audit")
