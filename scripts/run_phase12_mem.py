#!/usr/bin/env python3
"""Phase 12 physical capture: fpga_manager PL load + /dev/mem MMIO.
...
def capture_boot(bridge, output, index, args):
    ...
    with uart_path.open("xb") as raw:
        while time.monotonic() - started < args.timeout:
            ...
            raise TimeoutError("physical Phase 12 capture timed out")
    time.sleep(0.02)
...
def run(args):
    ...
    report = {"schema": "aster.phase12.physical-mem.v1", "status": "running",
    ...
    try:
        ...
        for index in range(1, args.boots + 1):
            ...
            print(f"PASS: physical Phase 12 boot={index} serial_bytes={entry['uart']['bytes']}", flush=True)
        bridge.stop()
        ...
        report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
        raise
    print(f"PASS: physical Phase 12 package safely STOPPED, {report_path}", flush=True)
...
def main():
    ...
    parser.add_argument("--name", default="streaming_ecg")
