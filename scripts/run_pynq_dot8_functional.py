#!/usr/bin/env python3
"""Guarded physical DOT8 two-hart C execution through Linux/PCAP only."""
import argparse
import hashlib
import json
...
from run_pynq_dot8 import artifact, guard_previous, state


def collect(bridge,reference,output,index,host_pause,timeout,entry):
    mmio = bridge.mmio; path = output/f"boot{index}.uart"
    with path.open("xb") as raw:
        ...
        while time.monotonic()-started < timeout:
            ...
            raw.flush()
            if not functional.UART.startswith(payload): raise RuntimeError("functional firmware reported wrong serial output")
            if mmio.read(4) & 0x1a: raise RuntimeError(f"functional core/serial error: {bridge.faults()}")
            if payload == functional.UART: break
            if count == 0: time.sleep(0.0001)
        else: raise TimeoutError("physical DOT8 functional job timed out")
    ...
    values = functional.validate_ram(data,reference["programs"]["dot8_runtime"],reference["configuration"]["l1"])
    entry.update(ram_values=values,reference_comparison=proof.comparison(values,reference,index),status="PASS")
    try: proof.validate_boot(entry,reference,output)
...
def run(args):
    ...
    output = args.output.resolve(); bench.require(not output.exists(),"functional output must be new; old evidence retained")
    ...
    raw = (args.reference.parent/reference["artifacts"]["dot8_runtime.hex"]["file"]).read_bytes()
    bench.require(hashlib.sha256(raw).hexdigest() == reference["metadata"]["firmware_sha256"],"functional ROM changed after preflight")
    ...
    print("PYNQ8_FUNCTIONAL: complete ELF/ROM/source/reset/signoff preflight passed",flush=True)
    import pynq
    ...
    report = dict(schema=proof.SCHEMA,status="running",board=pynq.Device.active_device.name,pynq_version=pynq.__version__,kernel=platform.release(),
        transport=physical.TRANSPORT,external_pmod_loopback=False,previous_bitstream=args.expected_loaded,previous_bitstream_sha256=args.expected_loaded_sha256,
        previous_state=previous,loaded_bitstream=str(bitstream),downloaded=args.download,clock_mhz=31.25,baud=115200,host_pause_seconds=args.host_pause,
        timeout_seconds=args.timeout,reference_sha256=reference_hash,overlay_sha256=overlay_hash,collector_revision=args.collector_revision,
        collector_files=sources,boots=[],final_state=None)
    ...
    try:
        ...
        if args.download:
            print("PYNQ8_FUNCTIONAL: downloading verified DOT8 overlay through Linux/PCAP",flush=True); active_overlay.download()
        ...
        for index in (1,2):
            ...
            collect(bridge,reference,output,index,args.host_pause,args.timeout,entry); save()
            print(f"PASS: physical Pynq-Z1 DOT8 functional cache={int(hardware['caches'])} boot={index} serial_bytes={entry['uart']['bytes']} "
                  "retained_RAM=65536 independent_functional_oracle exact_50_reference_counters",flush=True)
        bench.require(proof.identity() == sources and sha(args.reference) == reference_hash and sha(args.overlay) == overlay_hash,"functional input/collector changed during execution")
    ...
    print(f"PASS: physical DOT8 functional package audited and CPU/DMA/compute safely STOPPED, {path}",flush=True)
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host-pause",type=float,default=0.2); parser.add_argument("--timeout",type=float,default=120); args = parser.parse_args()
    try: run(args)
