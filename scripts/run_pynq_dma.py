#!/usr/bin/env python3
"""Collect actual paired CPU/DMA execution through PYNQ Linux, never JTAG.
...
def state(bridge):
    return dict(control_state(bridge),dma=bridge.dma_snapshot())
...
def guard_previous(pynq, expected_path, expected_sha):
    ...
    for dma in (False,True):
        for caches in (False,True):
            try: candidates.append(validate_handoff(hwh,2,expected_coherent=True,expected_cache=caches,expected_dma=dma))
            except ValueError: pass
    bench.require(len(candidates) == 1, "current overlay lacks a known coherent/DMA HWH")
    ...
    cls = DmaBridge if handoff.get("dma",False) else CoherentBridge
...
def collect_boot(bridge,reference,output,index,host_pause,timeout,entry):
    ...
    with path.open("xb") as raw:
        ...
        while time.monotonic()-started < timeout:
            ...
            if type(count) is not int or not 0 <= count <= 512: raise RuntimeError("invalid DMA serial FIFO count")
            ...
            raw.flush()
            if len(payload) > methods*65536: raise RuntimeError("oversized physical DMA serial stream")
            if mmio.read(4) & 0x1a: raise RuntimeError(f"physical core/serial error: {bridge.faults()}")
            if payload.count(b"\n") >= methods: break
            if count == 0: time.sleep(0.0001)
        else: raise TimeoutError("physical DMA workload timed out")
...
def run(args):
    ...
    print("PYNQ7: complete overlay/reset/signoff/ELF/ROM/reference preflight passed",flush=True)
    import pynq
    ...
    try:
        ...
        if args.download:
            print("PYNQ7: downloading verified DMA overlay through Linux/PCAP",flush=True); active_overlay.download()
        ...
        for index in range(1,reference["configuration"]["boots"]+1):
            ...
            print(f"PASS: physical Pynq-Z1 DMA boot={index} methods={len(entry['records'])} serial_bytes={entry['uart']['bytes']} "
    ...
    print(f"PASS: physical DMA package audited and CPU/DMA safely STOPPED, {path}",flush=True)
    return report
