#!/usr/bin/env python3
"""Phase 12.5 physical capture: fpga_manager PL load + /dev/mem MMIO.

Runs the machine-timer interval firmware on the Pynq-Z1 at 31.25 MHz, confirms
the reported TIMER PASS and bounded match latency, and snapshots the stopped RAM.
It must run as root.
"""
import argparse
import json
...
def capture_boot(bridge, output, index, args):
    ...
    with uart_path.open("xb") as raw:
        while time.monotonic() - started < args.timeout:
            ...
            if b"TIMER FAIL" in payload:
                raise RuntimeError("timer firmware reported failure")
            if b"TIMER PASS" in payload:
                break
            if count == 0:
                time.sleep(0.0001)
        else:
            raise TimeoutError("physical Phase 12.5 capture timed out")
    time.sleep(0.02)
    text = payload.decode("ascii")
    lines = text.splitlines()
    if "TIMER PASS" not in lines:
        raise RuntimeError("missing TIMER PASS record")
    latency = None
    for line in lines:
        match = re.fullmatch(r"MATCH_LATENCY=(\d+)", line)
        if match:
            latency = int(match.group(1))
    if latency is None or not 0 < latency < LATENCY_LIMIT:
        raise RuntimeError(f"invalid match latency {latency!r}")
    before = bridge.state()
    bridge.stop()
    ...
    with ram_path.open("xb") as stream:
        stream.write(ram)
    after = bridge.state()
    return {"boot": index, "status": "PASS", "match_latency_cycles": latency,
...
def run(args):
    ...
    report = {"schema": "aster.phase12.5.physical-mem.v1", "status": "running",
    ...
    try:
        ...
        for index in range(1, args.boots + 1):
            ...
            print(f"PASS: physical Phase 12.5 boot={index} latency={entry['match_latency_cycles']}",
                  flush=True)
        bridge.stop()
        ...
        report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
        raise
    print(f"PASS: physical Phase 12.5 package safely STOPPED, {report_path}", flush=True)
