#!/usr/bin/env python3
"""Read-only outer audit for the Phase 12.5 machine-timer closeout.
...
try:
    from .timer_study import audit_study, parse_unit
...
SCHEMA = "aster.phase12.5.closeout.v1"
SOURCE_SCHEMA = "aster.phase12.5.source.v1"
REVISION = "c3ec874d00155bd297a56dc55290fb7795651b49"
TOP = {"spec", "input", "simulation", "fpga", "physical", "source", "verification"}
REQUIREMENTS = {
    "01-contract-and-firmware": ["spec/timer.md", "input/timer_interval.c", "input/timer_interval.hex"],
...
def inventory(directory):
    require(directory.is_dir() and not directory.is_symlink(), "invalid Phase 12.5 closeout directory")
    names = {path.name for path in directory.iterdir()}
    require(TOP | {"README.md"} <= names <= TOP | {"README.md", "manifest.json"},
            "missing or extra Phase 12.5 top-level package")
    files = {}
    ...
    require(all(path in files for paths in REQUIREMENTS.values() for path in paths),
            "missing Phase 12.5 requirement evidence")
    return files


def audit_input(directory):
    source = (directory / "timer_interval.c").read_text()
    require("ASTER_TIMER_ABI" in source and "TIMER PASS" in source, "firmware source is not the timer interval test")
    words = [line for line in (directory / "timer_interval.hex").read_text().splitlines() if line]
    require(len(words) == 16384 and all(re.fullmatch(r"[0-9a-fA-F]{8}", word) for word in words),
            "timer ROM image is not a canonical 16384-word firmware")
...
def audit_physical(directory):
    ...
    require(report.get("schema") == "aster.phase12.5.physical-mem.v1" and report["status"] == "complete",
    ...
    for boot in report["boots"]:
        require("TIMER PASS" in boot["uart_output"], "physical boot did not report TIMER PASS")
        latency = boot["match_latency_cycles"]
        require(0 < latency < 512, "physical match latency is out of bounds")
    ...
    require(all(latency == latencies[0] for latency in latencies), "physical match latency is not reproducible")
    return {"boots": len(report["boots"]), "match_latency_cycles": latencies[0]}
...
def audit_logs(directory):
    ...
    for phrase in ("PASS: timer free-run, compare match, clear/enable, byte merge, 64-bit wrap and reset",
                   "PASS: coherent SoC timer harts=2 caches=1"):
...
def audit(directory, *, current=False):
    ...
    require(set(manifest) == {"schema", "status", "files", "requirements", "summary"} and
            ...
            "incomplete Phase 12.5 manifest")
...
def main():
    ...
    print("PASS: all Phase 12.5 requirements, independent study/physical audits and clean source")
